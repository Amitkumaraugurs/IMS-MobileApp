...
class InventoryConfirmationState extends State<InventoryConfirmation> {
  ...
  Future<List> loadStockItemList() async {
    ...
    return await api.Gettransfer(0, 7, 0, storeid, 0, "", 0, 0).then((result) {
...
//                    decoration: ,
          ...
          Container(
            child: Column(
                ...
                children: <Widget>[
                  Container(
                    child: Text(InventoryList[index].FromStore != null
                        ? InventoryList[index].FromStore
                        : ''),
                  ),
                  Container(
                    child: Text(InventoryList[index].ToStore != null
                        ? InventoryList[index].ToStore
                        : ''),
                  ),
                  Container(
                    child: Text(InventoryList[index].ItemDesc != null
                        ? InventoryList[index].ItemDesc
                        : ''),
                  ),
                  Container(
                    child: Text(InventoryList[index].Barcode != null
                        ? InventoryList[index].Barcode
                        : ''),
                  ),
                  Container(
                    child: Text(
                        InventoryList[index].IssuedQty.toString() != null
                            ? InventoryList[index].IssuedQty.toString()
                            : ''),
                  ),
                  Container(
                    child: Text(InventoryList[index].Status != null
                        ? InventoryList[index].Status
                        : ''),
                  ),
                ]),
          ),
